...
function getWeatherColor(value, variable) {
    const scale =
        weatherColorScales[variable] || weatherColorScales.temperature_2m;
...
const colorSchemes = {
    ylorrd: {
        ...
        gradient:
            "linear-gradient(to right, #FFEDA0, #FEB24C, #FD8D3C, #FC4E2A, #E31A1C, #BD0026, #800026)",
    },
    blues: {
        colors: [
            "#f7fbff",
        ...
        gradient:
            "linear-gradient(to right, #f7fbff, #deebf7, #c6dbef, #9ecae1, #6baed6, #3182bd, #08519c)",
    },
    greens: {
        colors: [
            "#f7fcf5",
        ...
        gradient:
            "linear-gradient(to right, #f7fcf5, #e5f5e0, #c7e9c0, #a1d99b, #74c476, #31a354, #006d2c)",
    },
    purples: {
        colors: [
            "#fcfbfd",
        ...
        gradient:
            "linear-gradient(to right, #fcfbfd, #efedf5, #dadaeb, #bcbddc, #9e9ac8, #756bb1, #54278f)",
    },
    viridis: {
        colors: [
            "#440154",
...
if (currentBasemap) {
    ...
    if (!activeBtn) {
        document
            .querySelectorAll('.basemap-btn[data-basemap="osm"]')
            .forEach((btn) => {
                btn.classList.add(
                    "active-basemap",
                    "border-blue-500",
...
function toggleLayerDetail(element, event) {
    ...
    if (
        event.target.closest(".layer-toggle") ||
            ...
            contents.forEach((c) => {
                ...
                if (c.id === targetId) {
                    ...
                    if (targetId === "analysis-tab") {
                        console.log(
                            "Switching to Analysis tab, reinforcing correlation init...",
                        );
...
document
    ...
    .addEventListener("change", async function (e) {
        ...
        console.log(
            `Toggle layer: ${folder}, type: ${layerType}, checked: ${toggle.checked}`,
        );
        ...
        if (toggle.checked) {
            ...
            if (layerType === "choropleth") {
                ...
                try {
                    ...
                    const choroplethRes = await fetch(
                        `/api/choropleth-data/${folder}`,
                    );
                    if (!choroplethRes.ok)
                        throw new Error("Failed to load choropleth data");
                    ...
                    if (hasYears) {
                        ...
                        document
                            .getElementById("yearSliderContainer")
                    ...
                    // Intermediate values
                    document.getElementById("legendMed").textContent =
                        formatNumber((minVal + maxVal) / 2);
                    document.getElementById("legendQ1").textContent =
                        formatNumber(minVal + (maxVal - minVal) * 0.25);
                    document.getElementById("legendQ3").textContent =
                        formatNumber(minVal + (maxVal - minVal) * 0.75);
                ...
                const layer = L.tileLayer(
                    `${storageBaseUrl}/${folder}/{z}/{x}/{y}.png`,
                    {
                        tms: false,
                        opacity: opacity,
                        maxZoom: 18,
...
function formatRegionName(code) {
    ...
    if (window.COUNTRY_MAPPING) {
        ...
        if (window.COUNTRY_MAPPING[code.toUpperCase()])
            return window.COUNTRY_MAPPING[code.toUpperCase()];
    }
...
function loadMoreRanking() {
    ...
    // Append new items
    const newHTML = generateRankingHTML(
        rankingState.rankings,
        startIndex,
        addCount,
    );
...
function toggleSortOrder() {
    ...
    if (rankingState.choroplethData) {
        rankingState.rankings = generateRankingData(
            rankingState.choroplethData,
            rankingState.year,
        );
    ...
    if (listEl) {
        ...
        listEl.innerHTML = generateRankingHTML(
            rankingState.rankings,
            0,
            initialCount,
        );
...
document.querySelectorAll(".color-scheme-picker").forEach((picker) => {
    picker.addEventListener("click", (e) => {
        ...
        picker
            .querySelectorAll(".color-scheme-btn")
        ...
        if (activeLayers[folder]) {
            ...
            if (layerInfo.type === "choropleth") {
                ...
                const currentYear =
                    years.length > 1 ? years[currentIndex] : "all";
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    window.handleMahaLabsClick = function (e, url) {
        e.preventDefault();
            ...
            if (cachedData && cachedData.length > 0) {
                ...
                if (closestPoint && closestDist < 15) {
                    ...
                    if (value !== null && value !== undefined) {
                        // Format time
                        const timeStr =
                            weatherTimeLabels[currentWeatherHourIndex] || "";
                        let formattedTime = "";
                        if (timeStr) {
                ...
                try {
                    ...
                    console.error(
                        "Error fetching weather for clicked location:",
                        err,
                    );
                }
            }
    ...
    try {
        ...
        if (data.success && data.layers) {
            ...
            if (data.layers.length === 0) {
                ...
                layerControls.innerHTML = data.layers
                    .map(
                        (layer) => `
...
document
    ...
    .addEventListener("click", refreshLayers);
...
map.on("moveend", function () {
    ...
    console.log(
        `Map View: [${center.lat.toFixed(4)}, ${center.lng.toFixed(4)}] @ Zoom ${zoom}`,
    );
});
...
async function handleWeatherToggle(checkbox) {
    ...
    console.log(
        `handleWeatherToggle called: ${variable}, checked: ${checkbox.checked}`,
    );
    ...
    if (checkbox.checked) {
        try {
            ...
            console.log(
                `Fetching weather for hour=${hour} (current hour: ${currentHour})`,
            );
...
async function fetchAndRenderWeather(variable, hourIndex = 0) {
    console.log(
        `>>> fetchAndRenderWeather OPTIMIZED: ${variable}, hour ${hourIndex}`,
    );
    ...
    try {
        ...
        console.log(
            `Weather data: ${weatherResult.point_count} points (${(performance.now() - startTime).toFixed(0)}ms)`,
        );
        ...
        if (!countriesGeoJSONCache) {
            ...
            if (!countriesResponse.ok) {
                console.error(
                    `Countries GeoJSON error: ${countriesResponse.status}`,
                );
                return;
            }
            ...
            console.log(
                `Countries cached: ${countriesGeoJSONCache.features.length} (centroids pre-calculated)`,
            );
        }
        ...
        countriesGeoJSONCache.features.forEach((feature, idx) => {
            ...
            weatherResult.data.forEach((point) => {
                const dist =
                    Math.pow(centroid.lat - point.lat, 2) +
                    Math.pow(centroid.lng - point.lon, 2);
                if (dist < closestDist) {
                    closestDist = dist;
        ...
        console.log(
            `Weather choropleth rendered in ${(performance.now() - startTime).toFixed(0)}ms`,
        );
            ...
            .addEventListener("click", function () {
                ...
                if (isPlaying) {
                    playInterval = setInterval(async () => {
                        const slider =
                            document.getElementById("weatherTimeSlider");
...
function renderWeatherForVariable(variable, data) {
    ...
    console.log(
        `Heat points: ${heatPoints.length}, checking if L.heatLayer exists: ${typeof L.heatLayer}`,
    );
    ...
    if (typeof L.heatLayer === "function") {
        ...
        console.error(
            "L.heatLayer is not available! Make sure leaflet-heat is loaded.",
        );
    }
...
function removeWeatherLayer(variable) {
    ...
    const hasOtherChoropleth = Object.values(activeLayers).some(
        (l) => l.type === "choropleth",
    );
...
weatherToggles.forEach((toggle) => {
    ...
    toggle.addEventListener("change", async function () {
        ...
        console.log(
            `Weather toggle changed: ${variable}, checked: ${this.checked}`,
        );
...
async function initCorrelationFeature() {
    ...
    try {
        ...
        correlationState.layers = data.layers.filter(
            (l) => l.layer_type === "choropleth",
        );
        console.log(
            `initCorrelationFeature: Found ${correlationState.layers.length} choropleth layers. Prefetching metadata...`,
        );
        ...
        if (select1) {
            ...
            console.log(
                "initCorrelationFeature: Preserved value detected:",
                preservedValue,
            );
            ...
            if (select1.value) {
                console.log(
                    "initCorrelationFeature: Auto-triggering update for restored value...",
                );
        ...
        const promises = correlationState.layers.map((layer) =>
            getLayerType(layer.folder_path),
        );
...
async function updateCorrelationLayer2() {
    ...
    if (!folder1) {
        select2.innerHTML =
            '<option value="">-- Pilih Layer 1 terlebih dahulu --</option>';
    ...
    if (compatibleCount === 0) {
        ...
        if (warningText)
            warningText.textContent = `Tidak ada layer ${type1 === "province" ? "provinsi" : "global"} lain yang tersedia.`;
    }
}
...
async function runCorrelationAnalysis() {
    ...
    try {
        ...
        const response = await fetch(
            `/api/correlation?layer1=${layer1}&layer2=${layer2}&year=${year}`,
        );
        const data = await response.json();
...
function displayCorrelationResults(data) {
    ...
    scoreBadge.className =
        "correlation-score-badge px-3 py-1 rounded-full text-xs font-bold";

    // Add color class based on score
    const absScore = Math.abs(score);
    if (absScore > 0.7) {
        scoreBadge.classList.add(
            score > 0 ? "positive-strong" : "negative-strong",
        );
    } else if (absScore > 0.3) {
        scoreBadge.classList.add(
            score > 0 ? "positive-moderate" : "negative-moderate",
        );
    } else {
    ...
    const strength =
        data.classification && data.classification.strength
            ? data.classification.strength
            : "Weak";
    ...
    const direction =
        data.classification && data.classification.direction
            ? data.classification.direction
            : "positive";
    directionBadge.textContent =
        direction.charAt(0).toUpperCase() + direction.slice(1);
    directionBadge.className =
        "inline-block ml-1 px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider";

    if (direction === "positive") {
        directionBadge.classList.add(
            "bg-green-50",
            "text-green-600",
            "border",
            "border-green-100",
    ...
    let html = text
        ...
        .replace(
            /\*\*(.*?):\*\*/g,
            '<div class="font-bold text-gray-800 dark:text-gray-200 mt-5 mb-2 border-b border-gray-100 pb-1 text-xs uppercase tracking-wide">$1</div>',
        )
        // Bold text (Strong/Weak/etc)
        .replace(
            /\*\*(.*?)\*\*/g,
            '<strong class="font-semibold text-indigo-600 dark:text-indigo-400">$1</strong>',
        )
        // Bullets: styled list items
        .replace(
            /• (.*?)(?=\n|$)/g,
            '<div class="flex items-start gap-2 ml-1 my-1.5"><span class="text-indigo-500 mt-0.5">•</span><span class="text-gray-600 dark:text-gray-400">$1</span></div>',
        )
    ...
    if (data.plotly_data && chartContainer) {
        ...
        document.querySelectorAll(".regression-btn").forEach((btn) => {
            ...
            newBtn.addEventListener("click", (e) => {
                ...
                // Update active state
                document
                    .querySelectorAll(".regression-btn")
                    .forEach((b) =>
                        b.classList.remove(
                            "active",
                            "bg-indigo-50",
